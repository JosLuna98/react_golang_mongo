...
describe('should work with axios mock', () => {
	...
	it('calls axios and creates a task', async () => {
		mockedAxios.post.mockImplementationOnce(() => Promise.resolve({ data: { error: false } }));

		const items = await Api.CreateTask('task_03');

		expect(items.data.error).toEqual(false);
	...
	it('calls axios and completes a task', async () => {
		mockedAxios.put.mockImplementationOnce(() => Promise.resolve({ data: { error: false } }));

		const items = await Api.CompleteTask('test_04');

		expect(items.data.error).toEqual(false);
		expect(mockedAxios.put).toHaveBeenCalledTimes(1);
		expect(mockedAxios.put).toHaveBeenCalledWith(BASE_API_URL + '/api/task/' + 'test_04', {
			headers: {
				'Content-Type': 'application/x-www-form-urlencoded'
			}
		});
